...
Consider multi-sig (Squads Protocol) for production authority.

### 7. No Emergency Pause
...
### 9. Solana 3.0 SPL Token Compatibility
**Status:** Resolved

The `@solana/spl-token` 0.4.x library changed the behavior of `createAccount()` to default to creating Associated Token Accounts (ATAs) when no keypair is provided. This caused issues with Solana 3.0's ATA program restrictions.

**Solution:** Integration tests now explicitly provide `Keypair` instances to `createAccount()` to create regular token accounts instead of ATAs. This ensures compatibility across Solana versions.

```typescript
// Correct usage for regular token accounts
...
const tokenAccount = await createAccount(
  ...
  mint,
  owner,
  keypair  // ← Explicit keypair creates regular account
);
```
